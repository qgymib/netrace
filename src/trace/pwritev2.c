...
static void s_decode_pwritev2_arg0(nt_strcat_t* sc, const nt_syscall_info_t* si)
{
    int fd = si->enter.entry.args[0];
    nt_strcat(sc, "%d, ", fd);
}

static void s_decode_pwritev2_arg1(nt_strcat_t* sc, const nt_syscall_info_t* si)
{
...
static void s_decode_pwritev2_arg2(nt_strcat_t* sc, const nt_syscall_info_t* si)
{
    int iovcnt = si->enter.entry.args[2];
    nt_strcat(sc, "%d, ", iovcnt);
}

static void s_decode_pwritev2_arg3(nt_strcat_t* sc, const nt_syscall_info_t* si)
{
    off_t offset = si->enter.entry.args[3];
    nt_strcat(sc, "%ld, ", (long)offset);
}

static void s_decode_pwritev2_arg4(nt_strcat_t* sc, const nt_syscall_info_t* si)
{
...
int nt_syscall_decode_pwritev2(const nt_syscall_info_t* si, int op, char* buff, size_t size)
{
    ...
    nt_strcat(&sc, "(");
    s_decode_pwritev2_arg0(&sc, si);
    s_decode_pwritev2_arg1(&sc, si);
    s_decode_pwritev2_arg2(&sc, si);
    s_decode_pwritev2_arg3(&sc, si);
    s_decode_pwritev2_arg4(&sc, si);
    nt_strcat(&sc, ") = ");
